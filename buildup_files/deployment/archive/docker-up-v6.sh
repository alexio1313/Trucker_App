#!/bin/bash
# AI TRUCK LOGISTICS PLATFORM - Docker Start Script (v6)
...
#   3. Trucker service missing routes (profile/trucks/earnings/history/journey/live-positions)
#   4. Social service MongoDB URI fix (password contains @)
#   5. Admin service: kyc, analytics, feature-flags, app (audit-logs), users routes
#   6. Swap truck_web to rebuilt image (nginx dynamic DNS + no-cache headers baked in)
#   7. Swap admin panel to rebuilt image (signout button)
...
# ── Swap web app to rebuilt image (has dynamic nginx + proper cache headers) ──
echo "  Ensuring web app uses rebuilt image (dynamic DNS + proper cache)..."
if docker image inspect truck_web_rebuilt >/dev/null 2>&1; then
  docker stop truck_web 2>/dev/null || true
  docker rm truck_web 2>/dev/null || true
  docker run -d \
    --name truck_web \
    ...
    truck_web_rebuilt
  echo "  Web app started with rebuilt image."
else
  echo "  WARN: truck_web_rebuilt image not found — using docker-compose default."
  echo "  Run: docker build -f apps/web/Dockerfile -t truck_web_rebuilt ."
fi

# ── Swap admin panel to rebuilt image (has signout button) ──────────────────
...
if [ -f "$AUTH_PATCH" ]; then
  echo "  [2/5] Patching trucker auth service..."
...
if [ -f "$TRUCKER_ROUTES_PATCH" ]; then
  echo "  [3/5] Patching trucker service routes..."
...
if [ -f "$MONGO_PATCH" ]; then
  echo "  [4/5] Patching social service MongoDB..."
...
if [ -f "$KYC_PATCH" ]; then
  ...
  [ -f "$APP_PATCH" ]       && docker cp "$APP_PATCH"        truck_admin_service:/app/dist/admin/app.js

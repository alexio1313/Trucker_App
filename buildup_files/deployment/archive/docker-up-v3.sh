...
echo "[4/4] Applying runtime patches and checking status..."
sleep 5

# Patch 1: API Gateway proxy fix (pathRewrite + body injection)
# This fixes login for all user types (admin/trucker/merchant).
# Root cause: Express strips /api/v1 prefix before proxy; express.json() consumes body before proxy can forward it.
PATCH_FILE="$SCRIPT_DIR/proxy.routes.patch.js"
if [ -f "$PATCH_FILE" ]; then
  echo "  Patching truck_api_gateway proxy routes..."
  docker cp "$PATCH_FILE" truck_api_gateway:/app/dist/routes/proxy.routes.js
  docker restart truck_api_gateway
  sleep 3
  echo "  Gateway patched."
else
  echo "  WARN: $PATCH_FILE not found - login may not work."
fi

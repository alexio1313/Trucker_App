// Journey management routes -- v2 fixed
'use strict';
var pg = require('pg');
...
module.exports = function registerJourneyRoutes(app) {
  ...
  // Create journey support tables if missing
  pool.query(
  ...
  function tid(req) {
    return req.headers['x-user-id'] || (req.user && (req.user.userId || req.user.user_id)) || null;
  }

  // GET /api/v1/truckers/my/active-load
  app.get('/api/v1/truckers/my/active-load', function(req, res) {
    var truckerId = tid(req);
    ...
    var sql = 'SELECT l.load_id, l.origin_city, l.dest_city, l.origin_address, l.dest_address,' +
      ' l.origin_lat::float AS origin_lat, l.origin_lng::float AS origin_lng,' +
      ' l.dest_lat::float AS dest_lat, l.dest_lng::float AS dest_lng,' +
      ' l.origin_state, l.dest_state, l.cargo_type, l.cargo_weight_kg::float,' +
    ...
    pool.query(sql, [truckerId, ['accepted', 'loading', 'in_transit']])
      .then(function(lr) {
        if (!lr.rows.length) {
        ...
        return Promise.all([
          ...
          res.json({
            ...
            data: {
              load: load,
              journey: results[0].rows[0] || null,
              fuelStops: results[1].rows
            }
          });
        });
      })
      .catch(function(e) {
        console.error('[active-load]', e.message);
        res.status(500).json({ success: false, error: { code: 'SERVER_ERROR', message: e.message } });
      });
  });

  // POST /api/v1/truckers/my/journey/begin-loading
  app.post('/api/v1/truckers/my/journey/begin-loading', function(req, res) {
    var truckerId = tid(req);
    if (!truckerId) return res.status(401).json({ success: false, error: { code: 'UNAUTHORIZED', message: 'Missing user id' } });
    var loadId = req.body && req.body.loadId;
    if (!loadId) return res.status(400).json({ success: false, error: { code: 'VALIDATION_ERROR', message: 'loadId required' } });

    pool.query('SELECT load_id, status FROM loads WHERE load_id = $1 AND trucker_id = $2', [loadId, truckerId])
      .then(function(chk) {
        if (!chk.rows.length) {
          res.status(404).json({ success: false, error: { code: 'NOT_FOUND', message: 'Load not found or not assigned to you' } });
          return null;
        }
        var cur = chk.rows[0].status;
        if (cur !== 'accepted') {
          res.status(400).json({ success: false, error: { code: 'INVALID_STATUS', message: 'Load must be accepted first. Current: ' + cur } });
          return null;
        }
        return pool.query("UPDATE loads SET status = 'loading', updated_at = NOW() WHERE load_id = $1", [loadId])
          .then(function() {
            res.json({ success: true, data: { loadId: loadId, status: 'loading', message: 'Arrived at pickup — cargo loading started' } });
          });
      })
      .catch(function(e) {
        console.error('[begin-loading]', e.message);
        res.status(500).json({ success: false, error: { code: 'SERVER_ERROR', message: e.message } });
      });
  });
  ...
  app.post('/api/v1/truckers/my/journey/start', function(req, res) {
    var truckerId = tid(req);
    var loadId = req.body && req.body.loadId;
    var startOdo = (req.body && req.body.startOdometerKm) || null;
    ...
    pool.query("SELECT load_id FROM loads WHERE load_id = $1 AND trucker_id = $2 AND status = ANY($3)",
      [loadId, truckerId, ['accepted', 'loading']])
      .then(function(ck) {
        if (!ck.rows.length) {
          res.status(404).json({ success: false, error: { message: 'Load not found or cannot be started now' } });
          return null;
        }
  ...
  app.post('/api/v1/truckers/my/journey/fuel-stop', function(req, res) {
    ...
    pool.query(
      ...
      [b.loadId, truckerId, b.fuelLiters, b.fuelCost, b.odometerKm || null, b.stationName || null]
    ).then(function(fr) {
      return pool.query(
      ...
      console.error('[fuel-stop]', e.message);
      res.status(500).json({ success: false, error: { message: e.message } });
    });
  });
  ...
  app.post('/api/v1/truckers/my/journey/deliver', function(req, res) {
    ...
    pool.query("UPDATE loads SET status = 'delivered', updated_at = NOW() WHERE load_id = $1 AND trucker_id = $2",
      ...
      .then(function() {
        return pool.query(
          "UPDATE journey_logs SET journey_status = $1, end_odometer_km = $2::numeric, actual_toll_cost = $3::numeric," +
  ...
  app.get('/api/v1/truckers/my/journey/stats', function(req, res) {
    var truckerId = tid(req);
    pool.query(
      "SELECT COUNT(*) FILTER (WHERE j.journey_status = 'completed') AS total_trips," +
  ...
  console.log('[journey] 6 endpoints registered: active-load, begin-loading, start, fuel-stop, deliver, stats');
};
